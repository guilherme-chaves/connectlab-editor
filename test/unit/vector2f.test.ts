/* eslint-disable @typescript-eslint/no-unsafe-assignment */
import { expect, test, beforeEach, describe } from 'vitest';

import Vector2f from '@connectlab-editor/types/vector2f';

let v1: Vector2f;
let v2: Vector2f;

describe('Teste de operações com vetores', () => {
  beforeEach(() => {
    v1 = new Vector2f(3, 3);
    v2 = new Vector2f(2, 2);
  });
  test('Criar vetor sem passar parâmetros', () => {
    const a = new Vector2f();
    expect(a).toEqual({ _x: 0, _y: 0, type: 'float' });
  });

  test('Criar vetor passando outro vetor como parâmetro', () => {
    const a = new Vector2f(v1);
    expect(a).toEqual(v1);
    expect(a).not.toBe(v1);
  });

  test('Criar vetor passando dois números como parâmetros', () => {
    const a = new Vector2f(10.5, 25);
    expect(a).toEqual({ _x: 10.5, _y: 25, type: 'float' });
  });

  test('Adicionar dois vetores', () => {
    expect(Vector2f.add(v1, v2)).toEqual({ _x: 5, _y: 5, type: 'float' });
  });

  test('Adicionar um valor escalar a um vetor', () => {
    expect(Vector2f.add(v1, 5)).toEqual({ _x: 8, _y: 8, type: 'float' });
  });

  test('Subtrair dois vetores', () => {
    expect(Vector2f.sub(v1, v2)).toEqual({ _x: 1, _y: 1, type: 'float' });
  });

  test('Subtrair um valor escalar de um vetor', () => {
    expect(Vector2f.sub(v1, 5)).toEqual({ _x: -2, _y: -2, type: 'float' });
  });

  test('Multiplicar dois vetores', () => {
    expect(Vector2f.mul(v1, v2)).toEqual({ _x: 6, _y: 6, type: 'float' });
  });

  test('Multiplicar vetor por valor escalar', () => {
    expect(Vector2f.mul(v1, 5)).toEqual({ _x: 15, _y: 15, type: 'float' });
  });

  test('Dividir dois vetores', () => {
    expect(Vector2f.div(v1, v2)).toEqual({ _x: 1.5, _y: 1.5, type: 'float' });
  });

  test('Dividir vetor por valor escalar', () => {
    expect(Vector2f.div(v1, 3)).toEqual({ _x: 1, _y: 1, type: 'float' });
  });

  test('Calcular produto escalar entre dois vetores', () => {
    expect(Vector2f.dot(v1, v2)).toBeCloseTo(12);
  });

  test('Calcular o produto vetorial entre dois vetores', () => {
    expect(Vector2f.cross(v1, v2)).toBeCloseTo(0);
  });

  test('Calcular o comprimento de um vetor', () => {
    expect(Vector2f.len(v1)).toBeCloseTo(Math.sqrt(18));
  });

  test('Calcular a interpolação linear entre dois vetores', () => {
    expect(Vector2f.lerp(new Vector2f(4, 4), v2, 0.5)).toEqual(
      { _x: 3, _y: 3, type: 'float' },
    );
  });

  test('Calcular a interpolação bilinear entre dois vetores', () => {
    expect(
      Vector2f.bilinear(
        new Vector2f(),
        new Vector2f(10, 10),
        new Vector2f(0.7, 0.3),
      ),
    ).toEqual({ _x: 7, _y: 3, type: 'float' });
  });

  test('Calcular a equalidade entre vetores', () => {
    expect(
      Vector2f.equals(
        new Vector2f(0.3 / 0.1, 0),
        new Vector2f(3.0, 0),
      ),
    ).toBe(true);
    expect(
      Vector2f.equals(
        new Vector2f(0.3 / 0.1, 0),
        new Vector2f(3.0, 0),
        0,
      ),
    ).toBe(false);
    expect(
      Vector2f.equals(
        new Vector2f(16, 16),
        new Vector2f(12, 12),
        5,
      ),
    ).toBe(true);
  });

  test('Calcular o mínimo entre dois vetores', () => {
    expect(Vector2f.min(new Vector2f(3, 0), new Vector2f(2, 5))).toEqual(
      { _x: 2, _y: 0, type: 'float' },
    );
  });

  test('Calcular o máximo entre dois vetores', () => {
    expect(Vector2f.max(new Vector2f(3, 0), new Vector2f(2, 5))).toEqual(
      { _x: 3, _y: 5, type: 'float' },
    );
  });

  test('Rotacionar vetor', () => {
    expect(Vector2f.rotate(v1, Math.PI)).toEqual({
      type: 'float',
      _x: expect.closeTo(-3),
      _y: expect.closeTo(-3),
    });
  });

  test('Calcular ângulo do vetor entre dois vetores', () => {
    expect(Vector2f.atan2(v2, v1)).toBeCloseTo(Math.PI / 4);
  });

  test('Normalizar vetor', () => {
    expect(Vector2f.normalize(new Vector2f(3, 4))).toEqual({
      type: 'float',
      _x: expect.closeTo(3 / 5),
      _y: expect.closeTo(4 / 5),
    });
  });

  test('Obter vetor em valores absolutos', () => {
    expect(Vector2f.abs(new Vector2f(-3, -76.2312))).toEqual({
      type: 'float',
      _x: expect.closeTo(3),
      _y: expect.closeTo(76.2312),
    });
  });

  test('Copiar valores de um vetor', () => {
    Vector2f.copy(v1, v2);
    expect(v2).toEqual(v1);
    expect(v2).not.toBe(v1); // Não deve ser o mesmo objeto
  });

  test('Clonar vetor para um novo objeto', () => {
    const a = new Vector2f(1.5, 1.5);
    const b = a.clone();
    expect(b).toEqual(a);
    expect(b).not.toBe(a); // Não deve ser o mesmo objeto
  });

  test('Adicionar vetor A ao produto do vetor B com o valor escalar S', () => {
    expect(
      Vector2f.madd(v1, v2, 10),
    ).toEqual({ _x: 23, _y: 23, type: 'float' });
  });
});
