...
export default class MouseEvents {
  ...
  onMouseClick(editor: Editor) {
    ...
    if (slotId.length > 0) {
      ...
      connectionEvents.addLine(editor, this, this._mouse.position);
    }
  ...
  clearDragCollisions() {
    this.collisionList.nodes.length = 0;
    this.collisionList.inputs.length = 0;
  }

  clearAllCollisions(editorEnv: EditorEnvironment) {
    this.clearUnselectedComponents(editorEnv);
    this.collisionList.nodes.length = 0;
    this.collisionList.slots.length = 0;
    this.collisionList.connections.length = 0;
    this.collisionList.texts.length = 0;
    this.collisionList.inputs.length = 0;
  }
}
